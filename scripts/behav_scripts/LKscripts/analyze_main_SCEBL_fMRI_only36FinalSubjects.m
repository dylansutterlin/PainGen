...
for sub = 1:numel(subjects)
   ...
   SCEBLmri_data.sub_TxTgrp{sub} = sub*ones(96, 1); % manually added 20/03/2024 for trial x t. df
   ...
   SCEBLmri_data.pain{sub} = dummy([1:16 18:33 35:50 52:67 69:84 86:101]); %len = 96
...
for sub = 1:numel(subjects)
   ...
   f = filenames('L*.xlsx', 'char');
   SCEBLmri_data.lc{sub}
   SCEBLmri_data.hc{sub}

   % type of learning cue 
...
cd(saveDir)
save SCEBLmri_learndata_FINAL_N36 SCEBLmri_data
%dataTable = struct2table(SCEBLmri_data, 'VariableNames', fieldnames(SCEBLmri_data));
%writetable(dataTable, 'SCEBLmri_data_FINAL_N36')
